...
install_dir=/storage/c1mota
runtime=$install_dir/c1lavax
root=$install_dir/os
font=$install_dir/LVM.bin
program=/Mota.lav
log=$install_dir/mota.log
lock=/tmp/c1mota.lock
game_pid=
launcher_pids=
launcher_suspended=0
launcher_frame=/tmp/c1mota-launcher.frame
...
resume_launcher() {
    ...
    [ -z "$launcher_pids" ] || kill -CONT $launcher_pids 2>/dev/null || true
    launcher_suspended=0
    launcher_pids=
}
...
tune_display() {
    if [ -r "$display_fast_only" ] && [ -w "$display_fast_only" ]; then
        ...
        if printf '%s' 1 > "$display_fast_only"; then fast_only_tuned=1; fi
    fi
    if [ -r "$display_refresh_max" ] && [ -w "$display_refresh_max" ]; then
        IFS= read -r saved_refresh_max < "$display_refresh_max"
        if printf '%s' 100 > "$display_refresh_max"; then refresh_max_tuned=1; fi
    fi
}
...
if [ ! -x "$runtime" ] || [ ! -r "$font" ] || [ ! -r "$root$program" ] || [ ! -r "$root/LavaData/MOTA.dat" ]; then
    printf '%s\n' 'Magic Tower installation is incomplete' >> "$log"
    exit 1
fi
...
printf '%s START program=%s stable=1\n' "$(date -Iseconds 2>/dev/null || date)" "$program" >> "$log"
C1LAVAX_ROOT="$root" C1LAVAX_FONT="$font" C1LAVAX_STABLE_FRAMES=1 C1LAVAX_MOTA_KEY_FIX=1 \
    "$runtime" "$program" >> "$log" 2>&1 &
game_pid=$!
wait "$game_pid"
status=$?
game_pid=
