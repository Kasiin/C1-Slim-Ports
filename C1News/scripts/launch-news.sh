#!/bin/sh
set -u
app=/storage/c1news/c1news
log=/storage/c1news/news.log
lock=/tmp/c1news.lock
...
# Private trust store; system files and certificate verification stay intact.
export SSL_CERT_FILE=/storage/c1news/cacert.pem
printf '%s START\n' "$(date -Iseconds)" >> "$log"
"$app" >> "$log" 2>&1 &
child=$!
wait "$child"
